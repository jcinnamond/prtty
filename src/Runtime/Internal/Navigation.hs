...
goBack = do
    env <- get
    let lastJump = max 1 $ fromMaybe env.pc env.lastJump
    let lastMarker = V.findIndexR isBackMarker $ V.slice 0 (lastJump - 1) env.instructions
    case lastMarker of
        (Just i) -> do
...
-- | End the presentation by moving to the last instruction
end :: Runtime
end = gets instructions >>= jump . V.length
